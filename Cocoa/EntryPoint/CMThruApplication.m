...
@implementation CMThruApplication

BOOL _shouldKeepRunning = YES;

+(CMThruApplication*)sharedInstance{
    return (CMThruApplication*)(CocoaApplication.sharedApplication);
}
...
-(id)init
{
    ...
    if(self)
    {
        NSLog(@"CMThruApplication::init");

#if TARGET_OS_OSX
        //Create Custom App Delegate Here To Avoid Having to Use .xib
        self.delegate = [CMThruApplicationDelegate sharedInstance];
#else
